...
// pgbovine
static VALUE
bind_cfp_get(VALUE bindval, VALUE sym)
{
    const rb_binding_t *bind;
    GetBindingPtr(bindval, bind);
    return INT2FIX(bind->cfp);
}
...
Init_Binding(void)
{
    ...
    rb_define_method(rb_cBinding, "frame_id", bind_cfp_get, 0); // pgbovine - get cfp address, which is a unique ID
